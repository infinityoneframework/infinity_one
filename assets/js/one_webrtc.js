...
(function() {
  ...
  OneChat.on_connect(function(one_chat, socket) {
    if (OneChat.ucxchat.webrtc_enabled) {
      console.log('webrtc on_connect');
      WebRTC.init(one_chat.socket, one_chat.ucxchat.user_id,
        one_chat.ucxchat.username);
    }
  });
  ...
  let WebRTC = {
    ...
    constraints: function() {
      console.log('InfinityOne', InfinityOne);
      var dm = false;
      console.log('DeviceManager', OneChat.DeviceManager);
      if (OneChat.DeviceManager) {
        dm = OneChat.DeviceManager;
      }
      console.log('dm', dm);
      if (dm) {
    ...
    setAudioElements: function() {
      ...
      // WebRTC.remoteAudio = InfinityOne.DeviceManager.get_device('handsfree_output_id')
    },
    init: function(socket, localName, localUsername) {
      ...
      var contraints_video_hs = {
        video: {
          optional: [{sourceId: OneChat.DeviceManager.get_device("video_input_id")}]
        },
        audio: {
          optional: [{sourceId: OneChat.DeviceManager.get_device("headset_input_id")}]
        }
      };
    ...
    startConnection: function() {
      ...
      console.log('startConnection', InfinityOne)
      if (WebRTC.hasUserMedia()) {
        trace('hasUserMedia')
        // var constraints = {
        //   video: {deviceid: {exact: InfinityOne.devicemanager.get_device("video_input_id")}},
        //   audio: {deviceid: {exact: InfinityOne.devicemanager.get_device("handsfree_input_id")}}
        // }
    ...
    setupPeerConnection: function(stream) {
      ...
      var configuration = {
        "iceServers": [{ "urls": window.InfinityOne.iceServers }]
      };
