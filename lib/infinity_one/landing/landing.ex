defmodule InfinityOne.Landing do
  @doc """
  ...
  defp subscribe_channel(%{user: user, channel: channel}) do
    OneChat.Subscription.create(%{channel_id: channel.id, user_id: user.id})
  end
  ...
  defp update_host(_changes, attrs) do
    ...
    endpoint =
      :infinity_one
      |> Application.get_env(InfinityOneWeb.Endpoint)
      |> put_in([:url, :host], host_name)

    Application.put_env :infinity_one, InfinityOneWeb.Endpoint, endpoint
  ...
  defp run_hooks(_changes, attrs) do
    {:ok, InfinityOne.Hooks.landing_update(attrs)}
  end
end
