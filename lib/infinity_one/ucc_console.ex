defmodule OneConsole do
  ...
  * users/1 - Get users by
  ...
  Get the subscription for a given username and channel name
      ...
      iex> OneConsole.subscription "admin", "general"

      %OneChat.Schema.Subscription{__meta__: #Ecto.Schema.Metadata<:loaded, "subscriptions">,
        alert: false,
  ...
  def subscription(username, room) do
    with %{} = user <- Accounts.get_by_username(username),
         %{} = channel <- OneChat.Channel.get_by(name: room) do
      OneChat.Subscription.get_by user_id: user.id, channel_id: channel.id
    end
  end
  ...
  * `:subs`
  ...
  Get channel by opts
  """
  def channel(opts), do: OneChat.Channel.get_by(opts)

  @doc """
  Get channels by
  """
  def channels(opts), do: OneChat.Channel.list_by(opts)

  @doc """
  Get message by opts.

  """
  def message(opts), do: OneChat.Message.get_by(opts)

  @doc """
  List messages by opts.
  """
  def messages(opts), do: OneChat.Message.list_by(opts)
end
