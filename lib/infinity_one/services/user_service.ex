defmodule InfinityOne.UserService do
  use InfinityOneWeb, :service
  ...
  defp do_callback(changes, params, opts) do
    if !!Application.get_env(:one_chat, :module, false) and
     opts[:callback] != false do
      (opts[:callback] ||
        &OneChat.Accounts.insert_user_default_channels/3).(changes, params, opts)
    else
      {:ok, changes}
    end
  end
