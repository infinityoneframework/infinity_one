...
defmodule InfinityOne.Logger.Formatter do
  @moduledoc ~S"""
  ...
  def format(config, level, msg, ts, md, config_md) do
    md = Keyword.take(md, config_md)
    format(config, level, msg, ts, md)
  end

  def console(level, msg, ts, md) do
    :logger
  ...
  defp output(:metadata, _, _, _, meta) do
    mod_sep = if meta[:function], do: ?., else: ?\s
    fun_sep = if meta[:line], do: ?:, else: ?\s
    Enum.map(meta, fn
      {:module, val} ->
        [metadata(val), mod_sep]
      {:function, val} ->
        [metadata(val), fun_sep]
      {:line, val} ->
        [metadata(val), ?\s]
      {key, val} ->
