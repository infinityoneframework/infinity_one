defmodule UcxUcc.Accounts.UserRole do
  ...
  Handles the many to many association between `UcxUcc.Accounts.User`
  and `UcxUcc.Accounts.Role`.
  ...
  * channel_id - for rooms scoped roles
  ...
  ## UccPubSub notifications

  Broadcasts the following `UcxUcc.UccPubSub` events for a user:
  ...
  * "role:delete", "channel:global" - when a global role is removed
  ...
  defp prepare_notify(%{action: :insert} = changeset) do
    ...
    if UccSettings.display_roles() and notify_role?(role.name) do
      ...
      UccPubSub.broadcast "role:insert", "channel:" <> scope, %{
  ...
  defp prepare_notify(%{action: :delete} = changeset) do
    ...
    if UccSettings.display_roles() and notify_role?(role.name) do
      ...
      UccPubSub.broadcast "role:delete", "channel:" <> scope, %{
        username: user.username,
        role: String.capitalize(role.name)
      }
    end
