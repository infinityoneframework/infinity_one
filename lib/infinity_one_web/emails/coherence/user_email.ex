...
defmodule InfinityOneWeb.Coherence.UserEmail do
  @moduledoc false
  use Phoenix.Swoosh, view: InfinityOneWeb.Coherence.EmailView, layout: {InfinityOneWeb.Coherence.LayoutView, :email}
  alias Swoosh.Email
  require Logger
  ...
  def welcome(user, url \\ InfinityOneWeb.Router.Helpers.url(InfinityOneWeb.Endpoint)) do
    %Email{}
    |> from(from_email())
    |> to(user_email(user))
    |> add_reply_to()
    |> subject(dgettext("coherence", "%{site_name} - Welcome to InfinityOne", site_name: site_name()))
