defmodule InfinityOneWeb.LandingChannel do
  @moduledoc """
  ...
  use Rebel.Channel, name: "landing", controllers: [
    InfinityOneWeb.LandingController
  ]
  ...
  def click_submit(socket, sender) do
    attrs = OneChat.ServiceHelpers.normalize_params(sender["form"])
    case InfinityOne.Landing.create(attrs) do
      {:ok, _} ->
  ...
  defp handle_submit_errors([_, changeset], socket) do
    Client.toastr socket, :error, InfinityOneWeb.Utils.format_errors(changeset)
  end
  ...
  defp validate(2, socket, sender) do
    attrs = OneChat.ServiceHelpers.normalize_params(sender["form"])["admin"]
  ...
  defp validate(3, socket, sender) do
    attrs = OneChat.ServiceHelpers.normalize_params(sender["form"])["default_channel"]

    case OneChat.Channel.change(Map.put(attrs, "user_id", Ecto.UUID.generate)) do
      %{valid?: true} ->
        true
      %{errors: errors} ->
  ...
  defp validate(4, socket, sender) do
    attrs = OneChat.ServiceHelpers.normalize_params(sender["form"])["email_from"]
    errors =
  ...
  def render_summary(socket, form) do
    fields = OneChat.ServiceHelpers.normalize_params(form)
    html =
      InfinityOneWeb.LandingView
