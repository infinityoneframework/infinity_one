defmodule OneChat.SideNavService do
  use OneChat.Shared, :service

  alias InfinityOne.Accounts
  alias OneChat.ServiceHelpers, as: Helpers
  alias OneChat.{ChatDat, Channel, ChannelService}
  alias OneChat.Schema.Direct, as: DirectSchema
  ...
  def render_rooms_list_seach(match, user_id, opts \\ [], channel_id \\ nil) do
    ...
    render_to_string OneChatWeb.SideNavView, "rooms_list.html", chatd: chatd
  end
  ...
  def render_more_channels(user_id) do
    ...
    render_to_string OneChatWeb.SideNavView, "list_combined_flex.html",
      channels: channels, current_user: user
  end

  def render_more_users(user_id) do
    {user, users} = get_more_users(user_id)
    render_to_string OneChatWeb.SideNavView, "list_users_flex.html",
      InfinityOne.Hooks.render_users_bindings([users: users, current_user: user])
  end
  ...
  def get_more_users(user_id) do
    ...
    users =
      ...
      |> filter_guest(user)
      |> InfinityOne.Hooks.process_user_subscription
      |> Enum.map(fn
        {user, nil} ->
