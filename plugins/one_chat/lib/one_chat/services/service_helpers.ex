defmodule OneChat.ServiceHelpers do
  # use OneChatWeb, :service
  ...
  alias OneChat.{AppConfig, Channel}
  ...
  def format_javascript_errors(errors) do
    ...
    |> Enum.map(fn {k, {msg, opts}} ->
      error = if count = opts[:count] do
        Gettext.dngettext(InfinityOneWeb.Gettext, "errors", msg, msg, count, opts)
      else
        Gettext.dgettext(InfinityOneWeb.Gettext, "errors", msg, opts)
      end
      {k, error}
    end)
    |> Enum.into(%{})
  ...
  def response_message(_channel_id, _body) do
    ...
    # body = OneChatWeb.MessageView.render("message_response_body.html", message: message)
  ...
  def get_bot_id do
    InfinityOne.Accounts.get_bot_id()
  end
  ...
  ## Examples
        ...
        iex> OneChat.ServiceHelpers.normalize_form_params(params)
  ...
  def normalize_form_params(params) do
    Enum.reduce params, %{}, fn
      ...
      %{"name" => field, "value" => value}, acc ->
        ...
        |> InfinityOne.Utils.deep_merge(acc)
    end
  end
  ...
  map.
    ...
    # Examples
      ...
      iex> OneChat.ServiceHelpers.normalize_params %{"_csrf" =>
  ...
  def broadcast_message(_body, _room, _user_id, _channel_id, _opts \\ []) do
    ...
    # OneChat.TypingAgent.stop_typing(channel_id, user_id)
  ...
  def show_sweet_dialog(socket, opts) do
    ...
    html = OneChatWeb.MasterView.render("sweet.html", opts: Map.put(opts, :show, true))
    |> safe_to_string
