defmodule OneChat.Message do
  @moduledoc """
  Context for application level message handling.

  Uses the OneModel module to add the standard database functions like:
  ...
  * and many more
  ...
  functions in this module. Checkout the `OneChat.Messages.create/`
  function for example.
  """
  use OneModel, schema: OneChat.Schema.Message
  use Timex

  alias InfinityOne.{Repo, Accounts, Permissions}
  alias OneChat.Schema.Channel, as: ChannelSchema
  alias OneChat.{Attachment, Channel, AppConfig, Mention, Subscription}
  ...
  See `OneChat.Attachment` and `OneChat.File` for more details.

  Upon successful database insertion, the `InfinityOne.OnePubSub` module is
  ...
  * `OneChatWeb.RoomChannel` - responsible for pushing the new message
     out to all users with the channel open.
  * `OneChat.Robot` - responsible for passing the message to all active bots
  ...
  defp do_create({:ok, %{message: %{type: "p"} = message}}) do
    ...
    OnePubSub.broadcast "message:private", "user_id:" <> message.user_id, %{message: message}
  ...
  defp do_create({:ok, %{message: message}}) do
    ...
    OnePubSub.broadcast "message:new", "channel:" <> message.channel_id, %{message: message, channel_name: message.channel.name}
    {:ok, message}
  end
  ...
  def update(message, attrs) do
    case super(message, attrs) do
      {:ok, message} ->
        ...
        OnePubSub.broadcast "message:update", "channel:" <> message.channel_id, %{message: message}
        {:ok, message}
      error ->
        error
    end
  ...
  def delete(message, %{user_roles: ur} = user) when is_list(ur) do
    if OneSettings.allow_message_deleting && (user.id == message.user_id ||
  ...
  def delete(%{attachments: atts} = message) when is_list(atts) do
    ...
    case message |> change() |> Repo.delete do
      {:ok, _} ->
        OnePubSub.broadcast "message:delete", "channel:" <>
  ...
  def sequential_message?(last_message, current_user_id, dt) do
    current_user_id == last_message.user_id and
      Timex.after?(Timex.shift(last_message.inserted_at,
        seconds: OneSettings.grouping_period_seconds()), dt)
  end
