defmodule OneChat.Schema.Message do
  @moduledoc """
  ...
  alias InfinityOne.{Accounts.User, OnePubSub}
  alias OneChat.Schema.{Mention, Channel, Reaction, Attachment, StarredMessage}
  ...
  def add_timestamp(%{data: %{timestamp: nil}} = changeset) do
    put_change(changeset, :timestamp, OneChat.ServiceHelpers.get_timestamp())
  end
  ...
  defp update_subscriptions(%{action: :insert, changes: changes} = changeset) do
    OnePubSub.broadcast "subscription:update", "message:insert",
      %{
