defmodule OneChat.Channel do
  use OneModel, schema: OneChat.Schema.Channel

  import Ecto.Changeset
  import InfinityOneWeb.Gettext

  alias InfinityOne.{Accounts, Permissions, Accounts.User, Repo, OnePubSub}
  alias OneChat.{Mute, Subscription}
    ...
    case super(changeset) do
      {:ok, _} = ok ->
        Enum.each Map.keys(changeset.changes), fn key ->
          OnePubSub.broadcast "user:all", "channel:change:key", %{
            key: key,
  ...
  def create_channel(name, user_id, params \\ %{}) do
    ...
    case OneChat.ChannelService.insert_channel user, params do
      {:ok, _} = ok ->
  ...
  def has_permission?(user, %{id: nil}, changes) do
  ...
  def get_authorized_channels(user_id) do
    user = OneChat.ServiceHelpers.get_user!(user_id)
    cond do
  ...
  def get_all_channels(user_id) do
    user_id
    |> OneChat.ServiceHelpers.get_user!
    |> get_all_channels
  end
