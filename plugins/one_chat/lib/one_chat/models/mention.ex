defmodule OneChat.Mention do
  use OneModel, schema: OneChat.Schema.Mention
  ...
  alias OneChat.{Subscription, Channel, Direct, Message}
  ...
  defp mention_all_users(poster, name, channel_id) do
    ...
    max = OneSettings.max_members_disable_notifications()
    ...
    case name do
      ...
      "all!" ->
        if has_permission?(poster, "mention-all!", channel_id) && Subscription.open_count(channel_id) <= max do
          OneChat.ChannelMonitor.get_users()
          |> Enum.map(fn user_id ->
