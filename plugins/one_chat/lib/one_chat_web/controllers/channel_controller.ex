defmodule OneChatWeb.ChannelController do
  use OneChatWeb, :controller
  use Rebel.Controller, channels: [
    OneChatWeb.UserChannel,
    OneChatWeb.RoomChannel,
  ] ++ InfinityOne.Hooks.one_chat_channel_controller_channels([])
  ...
  alias OneChat.{Message, Channel, Subscription, ChannelService}
  alias InfinityOne.{Accounts.User, Hooks, Accounts, Permissions}
  ...
  defp do_show(conn, channel, user) do
    OneChat.PresenceAgent.load user.id
    ...
    conn
    |> put_view(OneChatWeb.MasterView)
  ...
  def page(conn, params) do
    ...
    # OneChat.PresenceAgent.load user.id
    OneChatWeb.HomeController.index(conn, params)
  end
