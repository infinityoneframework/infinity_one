defmodule OneChatWeb.MasterView do
  use OneChatWeb, :view
  alias OneChat.{ChannelService, ChatDat}
  ...
  def loading, do: ""
  def get_mb(chatd), do: OneChatWeb.MessageView.get_mb(chatd)
  ...
  def cc(config, item) do
    if apply OneSettings, item, [config] do
      ""
    else
      " hidden"
    end
  ...
  def get_chat_settings(channel) do
    ...
    window.chat_settings = {
      ...
      accepted_media_types: '#{OneSettings.accepted_media_types()}',
      maximum_file_upload_size_kb: #{OneSettings.maximum_file_upload_size_kb()},
      protect_upload_files: #{OneSettings.protect_upload_files()},
    };
    OneChat.settings = chat_settings;
    """
  end
