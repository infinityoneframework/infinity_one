defmodule OneChatWeb.MessageView do
  @moduledoc """
  ...
  alias OneChat.{Message, Subscription, AttachmentService}
  ...
  def get_reaction_people(reaction, user) do
    OneChat.ReactionService.get_reaction_people_names(reaction, user)
  end
  ...
  def role_tags(message) do
    if OneSettings.display_roles() do
      message.user_id
      |> Helpers.get_user!
      |> InfinityOne.Accounts.User.tags(message.channel_id)
    else
      []
    end
  end
  ...
  def get_mb(chatd) do
    ...
    settings =
      [
        max_message_length: OneSettings.max_allowed_message_size(config),
        show_formatting_tips?: OneSettings.show_formatting_tips(config),
  ...
  * Processing built-in markup like ~strike~ formatting
  ...
  def pinning_allowed?(opts, config) do
    OneSettings.allow_message_pinning(config) &&
