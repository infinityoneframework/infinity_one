defmodule OneChatWeb.SharedView do
  use InfinityOne.Utils
  use InfinityOneWeb.Gettext

  import Phoenix.HTML.Tag, warn: false

  alias InfinityOne.{Permissions, Repo, Accounts, Accounts.User, Hooks}
  alias OneChat.{Subscription, ChatDat}
  ...
  def get_status(user) do
    OneChat.PresenceAgent.get(user.id)
  end
  ...
  def get_desktop_notifications_for do
    default = OneChat.NotificationSetting.get_system_name()
    [
  ...
  * user struct - Doses the following
  ...
  def avatar_url(username, _) do
    OneChat.AvatarService.avatar_url username
  end
  ...
  def get_available_capacity(settings) do
    case OneChat.Settings.FileUpload.get_disk_usage(settings) do
  ...
  def get_uploads_size(settings) do
    case OneChat.Settings.FileUpload.get_uploads_size(settings) do
  ...
  def input_description(field) do
    gettext("Your %{brand} administrator had disabled the changing of %{field}", brand: InfinityOne.brandname, field: field)
  end
