defmodule OneChatWeb.SystemChannel do
  use Phoenix.Channel
  use OneLogger

  alias InfinityOneWeb.Presence
  ...
  # alias OneChat.{Subscription, Repo, Flex, FlexBarService, ChannelService}
  # alias OneChat.{AccountView, Account, AdminService}
  # alias OneChat.ServiceHelpers, as: Helpers
  ...
  def handle_info(:after_join, socket) do
    ...
    OnePubSub.subscribe "status:" <> user_id
    {:noreply, socket}
  end

  def handle_info({"status:" <> _, "set:" <> status, _payload}, socket) do
    if status != "" do
      OneChat.PresenceAgent.put(socket.assigns.user_id, status)
      update_status socket, status
    end
    {:noreply, socket}
  end
  ...
  def update_status(%{assigns: %{user_id: user_id, username: username}} =
    socket, status) do
    case OneChat.PresenceAgent.get_and_update_presence(user_id, status) do
      ^status ->
