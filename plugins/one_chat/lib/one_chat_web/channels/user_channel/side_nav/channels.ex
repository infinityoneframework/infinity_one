defmodule OneChatWeb.UserChannel.SideNav.Channels do
  import Rebel.Core
  import Phoenix.View, only: [render_to_string: 3]
  import InfinityOneWeb.Gettext

  alias InfinityOneWeb.Query
  alias OneChat.{Channel, ChannelService}
  alias OneChatWeb.{SideNavView, RebelChannel.Client}
  ...
  defp render_channels(options, user_id, socket, _client) do
    ...
    html = Phoenix.View.render_to_string OneChatWeb.SideNavView,
  ...
  defp render_members_list(username, socket, sender) do
    ...
    users =
      ...
      |> Enum.take(6)
      |> Enum.map(& struct(&1, status: OneChat.PresenceAgent.get(&1.id)))
  ...
  defdelegate format_error(term), to: OneChatWeb.SharedView, as: :format_errors
  ...
  defp do_open_room(reply, socket) do
    ...
    |> Client.page_loading()
    |> Rebel.Core.async_js("""
      $('.page-loading-container').html(OneUtils.loading_animation());
      OneChat.roomManager.render_room(#{reply});
