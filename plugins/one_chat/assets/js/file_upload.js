...
class FileUpload {
  ...
  validate_upload(file) {
    let size = OneChat.settings.maximum_file_upload_size_kb * 1024
    if (file.size > size) {
      console.log('file sizes', file.size, size)
      toastr.error('File size exceeds the ' + OneChat.settings.maximum_file_upload_size_kb + 'KB maximum!')
      return false
    }
    if (!OneChat.fileUploadIsValidContentType(file.type)) {
