defmodule MscsWeb.FlexBar.Tab.Mscs do
  ...
  def open(socket, user_id, channel_id, tab, nil) do
    socket = super(socket, user_id, channel_id, tab, nil)
    exec_js(socket, "$('.flex-tab-container.opened section.flex-tab').css('width', '500px').css('background-color', 'black');Rebel.set_event_handlers('.mscs-container');")
    socket
  end

  def close(socket) do
    exec_js(socket, "$('.flex-tab-container.opened section.flex-tab').css('width', '400px')")
    super(socket)
  end
