defmodule UccAdminWeb.AdminChannel do
  ...
  defp render_members_list(pattern, socket, sender) do
    ...
    users =
      ("%" <> pattern <> "%")
      |> UcxUcc.Accounts.list_users_without_role_by_pattern(role_id, count: 8, scope: scope)
      |> Enum.map(& Map.put(&1, :status, UccChat.PresenceAgent.get(&1.id)))

    html = render_to_string UccChatWeb.AdminView, "permissions_users_autocomplete.html",
  ...
  defp update_member_list(socket, %{} = role, scope) do
    # Logger.warn ""
    html = render_to_string UccChatWeb.AdminView,
  ...
  def render_rooms_list(pattern, socket) do
    ...
    html = render_to_string UccChatWeb.AdminView, "permissions_rooms_autocomplete.html",
      [rooms: rooms]
  ...
  def select_room(room, socket, opts \\ []) do
    ...
    if channel = Channel.get_by name: room do
      ...
      html = render_to_string UccChatWeb.AdminView, "permissions_user_roles_container.html",
