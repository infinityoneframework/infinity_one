defmodule OneWebrtcWeb.WebrtcChannel do
  use InfinityOneWeb, :channel
  use Phoenix.Channel
  use OneLogger
  ...
  alias OneWebrtcWeb.FlexBar.Tab.MembersList
  alias OneWebrtcWeb.VideoView
  ...
  def device_manager_init(socket, _payload) do
    case exec_js(socket, "window.OneChat.DeviceManager.installed_devices") do
  ...
  def on_connect(socket) do
    ...
    case get_client_device socket do
      nil ->
        broadcast_js socket, "window.OneChat.devices = {};" <> ice_servers_js()
        socket
      device ->
        str =
        ...
        broadcast_js socket, "window.InfinityOne.devices = {" <> str <> "};" <> ice_servers_js()
        ...
        # spawn fn ->
        #   Process.sleep 500
        #   OnePubSub.broadcast "ip:" <> socket.assigns[:ip_address], "devices:ready"
        # end
        socket
    end
  end
  ...
  def ice_servers_js do
    servers_str =
      OneSettings.webrtc_servers()
      |> String.split(",", trim: true)
      |> Enum.map(&String.trim/1)
      |> inspect
    " window.InfinityOne.iceServers = #{servers_str};"
  end
  ...
  def handle_out("webrtc:leave" = ev, payload, socket) do
    ...
    unless payload[:dest] do
      ...
      InfinityOneWeb.Endpoint.broadcast CC.chan_user <> socket.assigns.state["otherName"], "webrtc:leave", %{}
    end
    {:noreply, socket}
  end
  ...
  def handle_in(ev = "webrtc:user-" <> nm, %{"type" => "offer", "name" => name,
    ...
    case socket.assigns[:state] do
      ...
      data ->
        ...
        |> one_broadcast("webrtc:offer", %{offer: offer, name: name, from: nm})
    end
    |> noreply
  end
    ...
    socket =
      case socket.assigns[:state] do
        ...
        data ->
          ...
          |> one_broadcast("webrtc:answer", %{anwser: answer, name: name})
      end
    {:noreply, socket}
  end
  ...
  def handle_in(ev = "webrtc:user-" <> _nm, %{"type" => "leave", "name" => name} =
    ...
    case socket.assigns[:state] do
      ...
      data ->
        ...
        |> one_broadcast("webrtc:leave", %{name: name})
    end
    {:noreply, socket}
  end
  ...
  def handle_in(ev = "webrtc:user-" <> _nm, %{"type" => "candidate",
    ...
    socket =
      ...
      |> one_broadcast("webrtc:candiate", %{candidate: candidate, name: name})
    {:noreply, socket}
  end
  ...
  defp do_broadcast(socket, name, message, data) do
    InfinityOneWeb.Endpoint.broadcast "webrtc:user-" <> name, "webrtc:" <> message, data
    socket
  end
  ...
  # defp one_broadcast(%{assigns: assigns} = socket, topic) do
  #   OnePubSub.broadcast "user:" <> assigns.user_id, topic, %{}
  #   socket
  # end

  defp one_broadcast(%{assigns: assigns} = socket, topic, payload) do
    OnePubSub.broadcast "user:" <> assigns.user_id, topic, payload
    socket
  end
  ...
  defp set_client_devices(nil, installed_devices, socket) do
    case ClientDevice.create %{user_id: socket.assigns.user_id, ip_addr: socket.assigns.ip_address} do
      {:ok, client_device} ->
        client_device
      {:error, changeset} ->
        errors = InfinityOneWeb.Utils.format_errors changeset
  ...
  defp set_client_devices(client_device, installed_devices, socket) do
    ...
    broadcast_js socket, "window.OneChat.DeviceManager.devices = {" <> str <> "}"
    socket
  end
