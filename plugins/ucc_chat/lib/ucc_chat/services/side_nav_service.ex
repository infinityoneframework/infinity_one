defmodule UccChat.SideNavService do
  ...
  alias UccChat.{ChatDat, Channel, ChannelService, Direct, Subscription}
  ...
  def render_more_users(user_id) do
    ...
    users =
      ...
      |> Enum.map(fn
        {user, nil} -> struct(user, subscription_hidden: nil, status: UccChat.PresenceAgent.get(user.id))
        {user, sub} -> struct(user, subscription_hidden: sub.hidden, status: UccChat.PresenceAgent.get(user.id))
      end)
