defmodule UccChat.Settings do
  ...
  [
    general: UccChat.Config.General,
    message: UccChat.Config.Message,
    layout: UccChat.Config.Layout,
    file_upload: UccChat.Config.FileUpload
  ]
  |> Enum.map(fn {field, mod} ->
    Enum.map mod.__schema__(:fields) -- [:id], fn name ->
      def unquote(name)() do
        UccChat.Settings.config()
