defmodule UccChat.Web.UserChannel do
  ...
  def join_room(user_id, room) do
    ...
    Endpoint.broadcast!(CC.chan_user() <> "#{user_id}", "room:join", %{room: room, user_id: user_id})
  end

  def leave_room(user_id, room) do
    Endpoint.broadcast!(CC.chan_user() <> "#{user_id}", "room:leave", %{room: room, user_id: user_id})
  end

  def notify_mention(%{user_id: user_id, channel_id: channel_id}, body) do
    Endpoint.broadcast!(CC.chan_user() <> "#{user_id}", "room:mention", %{channel_id: channel_id, user_id: user_id, body: body})
  end
  def user_state(user_id, state) do
    Endpoint.broadcast!(CC.chan_user() <> "#{user_id}", "user:state", %{state: state})
  end
