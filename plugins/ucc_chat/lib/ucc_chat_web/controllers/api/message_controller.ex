defmodule UccChatWeb.API.MessageController do
  ...
  def create(conn, params) do
    ...
    case Accounts.get_user user_id, preload: [:roles, user_roles: [:role]] do
      ...
      user ->
        ...
        body = params["text"]
        channel = UccChat.Channel.get_by(name: channel_name) |> IO.inspect(label: "channel")
        ...
        case Message.create(%{body: body, user_id: user.id, channel_id: channel.id}) do
          ...
          {:error, changeset} ->
            error_string = UccChatWeb.SharedView.format_errors(changeset)
